...
public class DialogAddOffice extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("لطفا کد آرایشگاه را وارد نمایید :");
        initViews();
        eventListener();
    ...
    private boolean checkField() {
        if (txt_officeid.getText().toString().trim().isEmpty()) {
            new MessageBox(context, "لطفا کد آرایشگاه را وارد نمایید .").show();
            return false;
        }
        if (checkIfExistOfficeId(Integer.valueOf(txt_officeid.getText().toString()))) {
            new MessageBox(context, "کد آرایشگاه تکراری می باشد .").show();
            return false;
        }
        return true;
    }
    ...
    private class AsyncInsertOffice extends AsyncTask<String, Void, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            dialog = ProgressDialog.show(context, "", "در حال ثبت آرایشگاه ...");
            dialog.show();
